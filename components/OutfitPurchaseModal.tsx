...
interface Props {
  ...
  /** Called when user wants to watch an ad for this outfit */
  onWatchAd?: (itemId: string, adsRequired: number) => Promise<{ adsWatched: number; adsRequired: number; unlocked: boolean; justUnlocked?: boolean } | null>;
}
    ...
    try {
      ...
      if (result) {
        ...
        if (result.unlocked || result.justUnlocked) {
          // Fully unlocked — trigger celebration
          handleBuy();
        }
      }
    } catch (e) { console.error('[OutfitAd]', e); }
    setWatchingAd(false);
  };
          ...
          {isUnlocked ? (
            ...
            <button
              ...
              style={{
                ...
                boxShadow: purchased ? 'none' : '0 6px 28px rgba(168,85,247,0.45)',
                color: purchased ? '#6b7280' : '#fff',
              }}
            >
