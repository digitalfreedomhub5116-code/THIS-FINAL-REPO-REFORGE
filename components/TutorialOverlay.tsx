    ...
    <>
        ...
        <style>{`
            @keyframes tutorial-pulse-aggressive {
                0% { box-shadow: 0 0 0 2px #00d4ff, 0 0 15px rgba(0,212,255,0.3); }
                50% { box-shadow: 0 0 0 2px #ffffff, 0 0 30px rgba(0,212,255,0.6); }
                100% { box-shadow: 0 0 0 2px #00d4ff, 0 0 15px rgba(0,212,255,0.3); }
            }
            .tutorial-highlight {
        ...
        <div className="fixed inset-0 z-[9999] pointer-events-none font-sans flex flex-col items-center justify-center">
            ...
            >
                <div className={`bg-[#0a0a0a] border rounded-xl shadow-2xl overflow-hidden flex flex-col transition-colors ${isError ? 'border-red-500 shadow-[0_0_30px_rgba(220,38,38,0.5)]' : analysisFailed ? 'border-amber-500 shadow-[0_0_30px_rgba(245,158,11,0.5)]' : 'border-system-neon shadow-[0_0_30px_rgba(0,212,255,0.3)]'}`}>
                                ...
                                <AnimatePresence mode="wait">
                                    {analysisFailed ? (
                                        ...
                                        <motion.button 
                                            ...
                                            animate={{ opacity: 1, scale: 1 }}
                                            onClick={onComplete}
                                            className="bg-system-neon text-black px-5 py-2 rounded font-bold font-mono text-xs hover:bg-white transition-colors flex items-center gap-2 shadow-[0_0_15px_rgba(0,212,255,0.4)]"
                                        >
                    ...
                    <div className="bg-black/50 py-1.5 px-4 flex gap-1 justify-center shrink-0">
                        {Array.from({ length: 6 }).map((_, i) => (
                            <motion.div 
                                key={i}
                                layout 
                                className={`h-1 rounded-full ${i === currentStep ? 'bg-system-neon shadow-[0_0_8px_#00d4ff]' : 'bg-gray-800'}`}
                                animate={{ 
