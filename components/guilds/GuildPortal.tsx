...
const PORTAL_TABS: { key: PortalTab; label: string; icon: React.ComponentType<any> }[] = [
...
const GuildPortal: React.FC<GuildPortalProps> = ({
  guild, myRole, myUserId, myName, myAvatarUrl, playerGold, onGoldChange, onExitPortal, onLeftGuild, onToast,
}) => {
  ...
  useEffect(() => {
    const unsub = subscribeToGuild(guild.id, {
      onKicked: (uid) => { if (uid === myUserId) { onToast?.('WARNING', 'You were removed from the guild'); onLeftRef.current(); } },
      onDisbanded: () => { onToast?.('WARNING', 'This guild was disbanded'); onLeftRef.current(); },
      onMissionComplete: (p) => { onToast?.('SUCCESS', 'Gate cleared!', p.title); setMissionSignal((n) => n + 1); },
    });
    return unsub;
  }, [guild.id, myUserId, onToast]);

  return (
    <div className="fixed inset-0 z-[60] flex flex-col" style={{ background: '#05060d' }}>
      {/* Portal header */}
      <div className="flex items-center gap-3 px-4 pt-[calc(12px+env(safe-area-inset-top,0px))] pb-3 border-b border-white/5" style={{ background: 'rgba(8,8,20,0.9)' }}>
      ...
      <div className="flex-1 min-h-0 relative">
        <AnimatePresence mode="wait">
          ...
          >
            {tab === 'info' && <GuildInfo guildId={guild.id} myUserId={myUserId} onLeft={onLeftGuild} onToast={onToast} />}
            {tab === 'gates' && <GuildGates guildId={guild.id} completionSignal={missionSignal} />}
            {tab === 'chat' && (
              <GuildChat
                guildId={guild.id}
                myUserId={myUserId}
                ...
                onKicked={() => { onToast?.('WARNING', 'You were removed from the guild'); onLeftGuild(); }}
                onDisbanded={() => { onToast?.('WARNING', 'This guild was disbanded'); onLeftGuild(); }}
                onMissionComplete={(p) => { onToast?.('SUCCESS', 'Gate cleared!', p.title); setMissionSignal((n) => n + 1); }}
              />
            )}
            {tab === 'war' && <GuildWar guildId={guild.id} myRole={myRole} onToast={onToast} />}
            {tab === 'vault' && <GuildVault guildId={guild.id} playerGold={playerGold} onGoldChange={onGoldChange} onToast={onToast} />}
          </motion.div>
        </AnimatePresence>
      </div>
      ...
      <div
        ...
        style={{ ...glassPanel, borderTop: '1px solid rgba(0,212,255,0.15)', borderRadius: 0 }}
      >
        {PORTAL_TABS.map((t) => {
          const active = tab === t.key;
          const Icon = t.icon;
          return (
            <button key={t.key} onClick={() => setTab(t.key)} className="relative flex flex-col items-center justify-center w-14 py-1 gap-1">
              {active && (
                <motion.div layoutId="portal-active" className="absolute -top-2 w-8 h-0.5 rounded-full" style={{ background: NEON, boxShadow: `0 0 8px ${NEON}` }} />
              )}
              <Icon size={20} style={{ color: active ? NEON : '#6b7280' }} />
              <span className="text-[9px] font-mono uppercase tracking-wider" style={{ color: active ? NEON : '#6b7280' }}>{t.label}</span>
            </button>
          );
        })}
      </div>
