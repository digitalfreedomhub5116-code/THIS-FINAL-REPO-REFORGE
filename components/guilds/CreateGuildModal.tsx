...
const CreateGuildModal: React.FC<CreateGuildModalProps> = ({
  ...
  useEffect(() => {
    ...
    if (!/^[A-Za-z0-9 _-]+$/.test(trimmed)) {
      setNameStatus("invalid");
      setNameError("Letters, numbers, spaces, - and _ only");
      return;
    }
  ...
  const confirmPurchase = async () => {
    ...
    if (gold < def.cost) {
      onToast?.(
        "WARNING",
        "Not enough gold",
        `You need ${def.cost.toLocaleString()} G for ${def.label}.`
      );
      setPurchaseConfirm(null);
      return;
    }
  ...
  const submit = async () => {
    ...
    if (gold < GUILD_CREATE_COST) {
      setErrorModal({
        ...
        message: `You need ${GUILD_CREATE_COST} gold to create a guild. You have ${gold.toLocaleString()}.`,
      });
      return;
    }
    setCreating(true);
    try {
      const { guild, player } = await createGuild({
        name: name.trim(),
        motto: motto.trim(),
        icon: iconKey,
        privacy,
      ...
      switch (err.code) {
        ...
        case "NAME_TAKEN":
          ...
          setErrorModal({
            title: "Name taken",
            message: "That guild name was just taken. Choose another.",
          });
          break;
        case "INSUFFICIENT_GOLD":
          if (typeof err.gold === "number") {
            setGold(err.gold);
            onGoldChange(err.gold);
          }
          onToast?.(
            "WARNING",
            "Not enough gold",
            "Earn more gold from quests and workouts."
          );
          onClose();
          break;
        default:
  ...
  const headerTitle =
    step === "review" ? "CONFIRM GUILD CREATION" : "CREATE YOUR GUILD";
        ...
        <div className="flex-1 overflow-y-auto px-5 pb-4">
          ...
          {step === 3 && (
            <div>
              <h3 className="text-white font-semibold mb-3">
                Choose Your Guild Icon
              </h3>
              <p className="text-[11px] text-gray-500 mb-3">
                Unlock more icons in the Guild Vault after your guild is
                founded.
              </p>
              <div className="grid grid-cols-2 gap-4">
                {GUILD_ICON_CATALOG.filter((ic) => ic.free).map((ic) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      {ic.free ? (
                        ...
                        <span
                          className="absolute top-1.5 right-1.5 text-[9px] font-bold px-1.5 py-0.5 rounded flex items-center gap-0.5"
                          style={{
                        ...
                        >
                          <Lock size={8} /> {ic.cost.toLocaleString()}G
                        </span>
                      )}
                ...
                {[
                  {
                    ...
                    emoji: "🔓",
                    title: "OPEN",
                    sub: "Anyone can join instantly",
                  },
                  {
                    key: "invite_only" as const,
                    emoji: "🔒",
                    title: "CLOSED",
                    sub: "Requires Guild Master approval",
                  },
                ].map((opt) => {
                    ...
                    <button
                      ...
                      style={{
                        ...
                        border: sel
                          ? `2px solid ${NEON}`
                          : "1px solid rgba(255,255,255,0.1)",
                        boxShadow: sel ? `0 0 14px ${NEON}44` : "none",
                      }}
                    >
                      <div
                        className="w-5 h-5 rounded-full flex-shrink-0 flex items-center justify-center"
                        style={{
                          border: `2px solid ${sel ? NEON : "#4b5563"}`,
                        }}
                      >
                        {sel && (
                          <div
          ...
          {step === "review" && (
            <div className="flex flex-col items-center text-center pt-2">
              <div
                className="w-28 h-28 rounded-3xl flex items-center justify-center mb-3 overflow-hidden"
                style={{
                  background: INPUT_BG,
              ...
              <div className="w-full space-y-2 text-sm">
                <Row
                  label="Type"
                  value={privacy === "open" ? "🔓 OPEN" : "🔒 CLOSED"}
                />
                <div className="flex items-center justify-between">
                  ...
                  <span
                    ...
                    style={{ color: "#fbbf24" }}
                  >
                    <Coins size={15} /> {GUILD_CREATE_COST} Gold
                  </span>
                </div>
                <Row
                  label="Your Balance"
                ...
                <Row
                  ...
                  value={`${afterBalance.toLocaleString()} G`}
                  muted
                />
              </div>
      ...
      <AnimatePresence>
        {purchaseConfirm && (
          ...
          >
            ...
            >
              <div
                className="w-20 h-20 mx-auto flex items-center justify-center mb-3 overflow-hidden"
                style={{
                  background: INPUT_BG,
                  borderRadius: "12px",
              ...
              <p className="text-white font-bold mb-1">
                Purchase {GUILD_ICON_BY_KEY[purchaseConfirm]?.label} Icon?
              </p>
              <p className="text-gray-400 text-sm mb-5 flex items-center justify-center gap-1">
                <Coins size={14} style={{ color: "#fbbf24" }} />{" "}
                {GUILD_ICON_BY_KEY[purchaseConfirm]?.cost.toLocaleString()} gold
              </p>
...
const Row: React.FC<{ label: string; value: string; muted?: boolean }> = ({
  ...
  <div className="flex items-center justify-between">
    ...
    <span
      className={muted ? "text-gray-400 font-mono" : "text-white font-semibold"}
    >
      {value}
    </span>
  </div>
