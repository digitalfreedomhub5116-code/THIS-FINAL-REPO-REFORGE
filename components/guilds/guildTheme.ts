...
export const glassPanel: React.CSSProperties = {
  background:
    "linear-gradient(180deg, rgba(255,255,255,0.05) 0%, rgba(8,8,20,0.85) 100%)",
...
export const neonGlow = (opacity = 0.5) =>
  `0 0 14px rgba(0,212,255,${opacity})`;
...
export function bannerStyle(banner?: string | null): React.CSSProperties {
  return {
    background:
      BANNER_GRADIENTS[banner || "gradient-cyan"] ||
      BANNER_GRADIENTS["gradient-cyan"],
  };
}

// ── Guild banner catalog (equippable in the Vault by Master/Vice) ──
...
export const GUILD_BANNER_CATALOG: GuildBannerDef[] = [
  {
    key: "gradient-cyan",
    label: "Azure",
    gradient: BANNER_GRADIENTS["gradient-cyan"],
  },
  {
    key: "gradient-violet",
    label: "Amethyst",
    gradient: BANNER_GRADIENTS["gradient-violet"],
  },
  {
    key: "gradient-crimson",
    label: "Crimson",
    gradient: BANNER_GRADIENTS["gradient-crimson"],
  },
  {
    key: "gradient-emerald",
    label: "Emerald",
    gradient: BANNER_GRADIENTS["gradient-emerald"],
  },
  {
    key: "gradient-gold",
    label: "Royal",
    gradient: BANNER_GRADIENTS["gradient-gold"],
  },
];
...
export const GUILD_ICON_BY_KEY: Record<string, GuildIconDef> =
  GUILD_ICON_CATALOG.reduce((m, d) => {
    m[d.key] = d;
    return m;
  }, {} as Record<string, GuildIconDef>);
