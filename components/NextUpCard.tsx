...
const SLOT_ICONS: Record<string, React.ReactNode> = {
  ...
  QUEST: <Target className="w-3.5 h-3.5 text-[#00d4ff]" />,
  MEAL: <Utensils className="w-3.5 h-3.5 text-green-400" />,
  FREE: <Zap className="w-3.5 h-3.5 text-[#00d4ff]" />,
};

const SLOT_COLORS: Record<string, string> = {
  SLEEP: '#00d4ff', ROUTINE: '#fb923c', BLOCKED: '#6b7280',
  WORKOUT: '#f87171', QUEST: '#00d4ff', MEAL: '#4ade80', FREE: '#33dfff',
};
...
export default function NextUpCard({ scheduleProfile, dailySchedule, quests, goals, onNavigateToQuests }: NextUpCardProps) {
    ...
    quests
      ...
      .forEach(q => {
        ...
        result.push({
          ...
          label: q.title,
          type: 'QUEST',
          icon: <Target className="w-3.5 h-3.5 text-[#00d4ff]" />,
          isQuest: true,
        });
      });
  ...
  return (
    ...
    >
      {/* Top accent */}
      <div className="h-0.5" style={{ background: 'linear-gradient(90deg, #00d4ff44, transparent 60%)' }} />
          ...
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5">
                ...
                />
                <span className="text-[10px] font-mono text-[#00d4ff] font-bold uppercase">Now</span>
              </div>
