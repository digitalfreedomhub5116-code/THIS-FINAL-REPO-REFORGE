          ...
          <>
                  ...
                  {filtered.map((plan, idx) => {
                    ...
                    return (
                      <motion.button
                        ...
                        className={`w-full text-left ${dc.bg} border ${isRecommended ? 'border-system-neon/70 ring-1 ring-system-neon/30' : hoveredId === plan.id ? 'border-system-neon/60 shadow-[0_0_20px_rgba(0,212,255,0.1)]' : dc.border} rounded-2xl p-5 transition-all duration-300 group relative`}
                      >
                        {isRecommended && (
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2 mb-1">
                ...
                <span className="text-[10px] text-[#00d4ff] font-bold uppercase tracking-widest">AI Personalized</span>
              </div>
              ...
              <motion.button
                ...
                className="w-full text-left bg-gradient-to-br from-purple-900/20 to-indigo-900/10 border border-purple-800/30 hover:border-purple-600/50 rounded-2xl p-5 transition-all duration-300 group disabled:opacity-60 disabled:cursor-not-allowed shadow-[0_0_15px_rgba(0,212,255,0.03)]"
              >
                <div className="flex items-start justify-between gap-3">
                  <div className="flex-1">
                    <div className="flex items-center gap-2 mb-2">
                      <span className="text-[9px] font-black px-2 py-0.5 rounded-full bg-purple-900/60 text-[#00d4ff]">AI GENERATED</span>
                    </div>
                    ...
                    {!isGenerating && healthProfile && (
                      <div className="flex gap-3 flex-wrap">
                        {healthProfile.goal && <span className="text-[9px] text-[#00d4ff]/70 font-mono bg-purple-900/20 px-2 py-0.5 rounded">{healthProfile.goal}</span>}
                        {healthProfile.equipment && <span className="text-[9px] text-[#00d4ff]/70 font-mono bg-purple-900/20 px-2 py-0.5 rounded">{equipmentLabel(healthProfile.equipment)}</span>}
                      </div>
                    )}
                    {isGenerating && (
                  ...
                  <div className="shrink-0 mt-1 text-[#00d4ff] group-hover:translate-x-1 transition-transform">
