...
const RadarTooltip = ({ active, payload, label }: any) => {
  if (active && payload && payload.length) {
    ...
    return (
      <div className="bg-black/95 border border-system-neon/50 backdrop-blur-xl p-4 rounded-lg shadow-[0_0_30px_rgba(0,212,255,0.2)] z-50 pointer-events-none min-w-[140px]">
...
const CustomRadarChart = ({ data, domainMax, onHover, color1, color2, fillOpacity = 0.4, compact = false }: any) => {
    ...
    const c1 = color1 || "#00d4ff";
    const c2 = color2 || "#00d4ff";
  ...
  return (
    <div className={`w-full h-full relative flex flex-col ${compact ? '' : ''}`}>
      ...
      <div className={`flex-1 relative min-h-[${compact ? '240px' : '400px'}] ${compact ? '' : 'bg-black/20 border border-white/5 rounded-lg overflow-hidden group'}`}>
        ...
        {!compact && (
            <motion.div 
                className="absolute left-0 w-full h-[2px] bg-gradient-to-r from-transparent via-system-neon/20 to-transparent shadow-[0_0_10px_rgba(0,212,255,0.2)] z-0 pointer-events-none"
                animate={{ top: ['0%', '100%', '0%'] }}
