...
import {
  ...
  Dumbbell, CheckCircle2, XCircle, Sparkles, Target
} from 'lucide-react';
    ...
    <div className="space-y-4">
      ...
      <div className="relative overflow-hidden rounded-2xl border border-red-900/40 bg-[#0d0000]"
        ...
        <div className="relative z-10 p-5 flex items-center gap-4">
          ...
          <button
            onClick={() => { onEnterDungeon?.(); onNavigate('CASTLE'); }}
            disabled={!isDungeonReady}
            className={`flex-shrink-0 px-4 py-3 rounded-xl font-mono font-black text-xs tracking-widest flex flex-col items-center gap-1 transition-all ${
              isDungeonReady
                ? 'bg-red-600 text-white border border-red-500 hover:bg-red-500 shadow-[0_0_20px_rgba(239,68,68,0.4)] hover:shadow-[0_0_30px_rgba(239,68,68,0.6)] active:scale-95'
                : 'bg-gray-900 text-gray-600 border border-gray-800 cursor-not-allowed'
            }`}
          >
            <Sword size={16} className={isDungeonReady ? 'text-white' : 'text-gray-700'} />
            <span>{isDungeonReady ? 'ENTER' : 'SEALED'}</span>
          </button>
        </div>
      </div>
